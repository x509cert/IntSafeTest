...
// Remove unnecessary include of assert.h to avoid ambiguous call to
// overloaded function (xcp\core\text\glyph\truetype\subpixel.cpp)
// #include <assert.h>
...
// +++ SILVERLIGHT_CUSTOMIZATION
// Added __LP64__ support.
#if defined(NEEDS_INT_DEFINED)
#define __int8  char
...
namespace SafeIntInternal
{
   ...
   public:
...
#endif
       {
           ...
           // +++ SILVERLIGHT CUSTOMIZATION
#if defined __INCLUDE_PAL_CORE__
           XAML_FAIL_FAST();
#else
           // Instead of throwing a C++ exception, dereference null to cause a crash
...
// TODO: doens't work to use intrinsics on amd64
...
// +++ SILVERLIGHT_CUSTOMIZATION
// Added __LP64__ path to avoid gcc error when casting from __int64 to T.
#ifdef __LP64__
    template < typename E >
    static void MultiplyThrow( const T& t, const U& u, T& ret )
#else
...
// --- SILVERLIGHT_CUSTOMIZATION
    ...
    // +++ SILVERLIGHT CUSTOMIZATION
    // Comment-out two operator overloads because they interfere with functions
    // that take a SafeInt* (xcp\core\text\glyph\truetype\fsglue.cpp)
...
// Only available in thunked directories/projects
...
// +++ SILVERLIGHT CUSTOMIZATION
// Custom operator overload for scenarios in xcp\core\text\Glyph\TrueType
template <typename T, typename U, class E>
T* operator +(T* lhs, SafeInt<U, E> rhs)
{
    // Note that lhs value is copied when being passed to this function,
    // unlike in operator += implementation. This allows delegating to += without side effects.
    return lhs += rhs;
}
// --- SILVERLIGHT CUSTOMIZATION
